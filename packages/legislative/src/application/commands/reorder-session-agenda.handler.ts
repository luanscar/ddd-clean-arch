...
export interface ReorderSessionAgendaCommand {
  sessionId: UniqueEntityId
  orderedItemIds: UniqueEntityId[]
  tenantId?: string
}

export class ReorderSessionAgendaHandler {
  constructor(
    private readonly sessionRepository: IDeliberativeSessionRepository,
    private readonly tenantProvider: ITenantProvider,
  ) {}

  async execute(command: ReorderSessionAgendaCommand): Promise<Result<void, DomainError>> {
    ...
    const now = new Date()
    const r = session.reorderAgendaItems(command.orderedItemIds, now)
    if (!r.ok) {
      return r
    }
