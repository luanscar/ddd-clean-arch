...
export interface ChangeVoteCommand {
...
export class ChangeVoteHandler {
  constructor(
  ...
  async execute(command: ChangeVoteCommand): Promise<Result<void, DomainError>> {
    ...
    const voteResult = poll.changeVote(command.voterId, command.optionRaw, this.clock.now())
